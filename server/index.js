...
function initializeGenderStreams() {
  ...
  maleStream.pipe(maleGzipStream).pipe(fs.createWriteStream("./gzip/male.csv.gz"));
  femaleStream.pipe(femaleGzipStream).pipe(fs.createWriteStream("./gzip/female.csv.gz"));
...
app.post("/upload", upload.single("file"), (req, res) => {
  ...
  const { maleStream, femaleStream, maleGzipStream, femaleGzipStream } = initializeGenderStreams();
  let headers = null;
  ...
  const splitterTransform = new Transform({
    ...
    transform(chunk, encoding, callback) {
      const data = this.remainingData ? this.remainingData + chunk.toString() : chunk.toString();
      ...
      lines.forEach(line => {
        ...
        else if (columns[genderIndex] === "female") femaleStream.write(line + "\n");
      });
    ...
    flush(callback) {
      if (this.remainingData) {
        ...
        const genderIndex = headers.indexOf("gender");
        if (columns[genderIndex] === "male") maleStream.write(this.remainingData + "\n");
        else if (columns[genderIndex] === "female") femaleStream.write(this.remainingData + "\n");
      }
  ...
  [maleGzipStream, femaleGzipStream].forEach((gzipStream, index) => {
    ...
    gzipStream.on("data", chunk => updateProgress(type, (gzipStream.bytesWritten / processedSize) * 100));
  ...
  pipeline(
    ...
    err => {
      if (err) {
        ...
        cleanupStreams(maleStream, femaleStream, maleGzipStream, femaleGzipStream, progressStream);
...
function sendFilesToClient(res, maleGzipStream, femaleGzipStream) {
  ...
  res.setHeader("Content-Disposition", "attachment; filename=processed-files.zip");
